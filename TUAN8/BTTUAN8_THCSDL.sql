...
-- Cải thiện:
	-- Thêm ràng buộc sđt chỉ được nhập ký tự số và có 11 hoặc 10 chữ số
	-- Thêm ràng buộc email ký tự đầu phải là chữ cái và có chứa ký tự @
	-- Thêm ràng buộc số fax chỉ được nhập số
...
VALUES
    ...
    ('HD0002', 'KH0002', 'NV0002', '2023-12-08', '2023-12-15', '2023-12-12',NULL),
    ...
    ('HD0005', 'KH0005', 'NV0005', GETDATE(), DATEADD(DAY, 1, GETDATE()), NULL, N'Địa điểm giao hàng E'),
    ...
    ('HD0008', 'KH0008', 'NV0006', GETDATE(), DATEADD(DAY, 5, GETDATE()), NULL, N'Địa điểm giao hàng H'),
    ('HD0009', 'KH0009', 'NV0008', GETDATE(), DATEADD(DAY, 2, GETDATE()), DATEADD(DAY, 3, GETDATE()), NULL),
...
VALUES
    ...
    ('CT0006', N'CÔNG TY VINAMILK', N'GIAO DỊCH 6', N'26_NGUYỄN VĂN LINH', 'CONTYVINAMILK@GMAIL.COM', '0128456722', '194132127'),
...
VALUES ('MH0001', N'iPhone 16', 'CT0001', 'LH0001', 400, N'Cái', 160000000),
		...
		('MH0004', N'Sửa tươi không đường', 'CT0006', 'LH0009', 100, N'Lốc', 28000),
...
from DONDATHANG
	...
	/* d: Cập nhật lại dữ liệu trong bảng KHACHHANG sao cho nếu tên công ty và tên giao dịch 
	của khách hàng trùng với tên công ty và tên giao dịch của một nhà cung cấp nào đó thì địa chỉ,
	điện thoại, fax và e-mail phải giống nhau.
	*/
		-- CẬP NHẬT DỮ LIỆU
UPDATE KHACHHANG
SET 
...
ON KHACHHANG.TENCONGTY = NHACUNGCAP.TENCONGTY
AND KHACHHANG.TENGIAODICH = NHACUNGCAP.TENGIAODICH;
		-- HIỂN THỊ ĐỂ KIỂM TRA KẾT QUẢ
select *
from KHACHHANG

	--e: Tăng lương lên gấp rưỡi cho những nhân viên bán được số lượng hàng nhiều hơn 100 trong năm 2024
		-- CẬP NHẬT DỮ LIỆU
UPDATE NHANVIEN
...
WHERE MANHANVIEN IN (
    ...
    WHERE CHITIETDATHANG.SOLUONG > 100
    AND YEAR(DONDATHANG.NGAYDATHANG) = 2024
);
		-- HIỂN THỊ ĐỂ KIỂM TRA KẾT QUẢ
SELECT *
FROM NHANVIEN
